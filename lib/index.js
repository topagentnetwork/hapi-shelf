  ...
  register: async (server, options) => {
    ...
    try {
      bookshelf = require('bookshelf')(require('knex')(options.knex))
       ...
       throw (new Error('Invalid knex options: ' + error.toString()))
    }

    Hoek.merge(bookshelf.Model.prototype, R.pick(['parse', 'format'], internals))
    ...
    Hoek.merge(
      bookshelf,
      R.zipObj(R.keys(methods), R.map(R.flip(R.bind)(bookshelf), R.values(methods)))
    )
